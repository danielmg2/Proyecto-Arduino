...
class SensorDao extends FactoryBD implements DAO{
    ...
    public static function findById($id){
        $sql = 'select * from sensor where id_arduino = ?;';
        $datos = array($id);
        $devuelve = parent::ejecuta($sql,$datos);
    ...
    public static function findByRecurso($recurso){
        $sql = 'select id_arduino,'.$recurso.',fecha from sensor;';
        $datos = array();
        $devuelve = parent::ejecuta($sql,$datos);
        $obj = $devuelve->fetchAll(PDO::FETCH_ASSOC);
        if($obj){
    ...
    public static function findRecursoByDays($recurso,$fecha1,$fecha2){
        ...
        $sql=" select id_arduino,".$recurso." from sensor where fecha between ? and ?;";
        $datos = array($fecha1,$fecha2);
        $devuelve = parent::ejecuta($sql,$datos);
        $arrayActuadores= array();
        while($obj = $devuelve->fetchObject()){         
